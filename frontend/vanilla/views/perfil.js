...
`;
    ...
    cargarPreferenciaTransporteFavorito();
...
async function cargarPreferenciaTransporteFavorito() {
    const correoUsuario = localStorage.getItem('correoUsuario');
    const contenedorPreferencias = document.getElementById('infoPreferencias');
    if (!correoUsuario || !contenedorPreferencias) return;
    try {
        const response = await fetch(`http://localhost:8080/api/rutas/historial?correo=${correoUsuario}`);
        if (response.ok) {
            const data = await response.json();
            if (!data || data.length === 0) {
                contenedorPreferencias.innerHTML = `
                  <div class="preference-item">
                      <span class="status-icon green">✔</span> <span>Metro / Integrados (Predeterminado)</span>
                  </div>
              `;
                return;
            }
            ...
            data.forEach(item => {
                if (item.transporte) {
                    const t = item.transporte.charAt(0).toUpperCase() + item.transporte.slice(1).toLowerCase();
            ...
            let transporteFavorito = Object.keys(conteo).reduce((a, b) => conteo[a] > conteo[b] ? a : b);
            contenedorPreferencias.innerHTML = `
              <div class="preference-item">
                  <span class="status-icon green">✔</span> <span>${transporteFavorito} (Más utilizado)</span>
              </div>
          `;
        }
    } catch (error) {
        console.error("Error al calcular la preferencia de transporte:", error);
    }
}