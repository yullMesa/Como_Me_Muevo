export function renderInicio(container) {
    ...
    <div class="inicio-wrapper" style="font-family: sans-serif; color: #1f2937; padding-bottom: 40px;">
        ...
        <!-- SECCIÓN INFERIOR: Widgets de Acceso Dinámico -->
  ...
  `;
    ...
    // Cargar dinámicamente el estado de las líneas según la hora
...
function calcularEstadoLineasPorHora() {
    ...
    const hora = fechaActual.getHours(); // Toma solo la hora (0 a 23)
    ...
    // Validación estricta: Antes de las 7 am (hora < 7) o después de las 8 pm (hora >= 20)
    ...
    const htmlLineas = lineas.map((linea, index) => {
        ...
        if (fueraDeServicio) {
            ...
            // Generar un estado aleatorio basado exclusivamente en la hora actual y el índice de la línea
            // Usamos la hora como semilla sumándole el índice para que cada línea tenga un comportamiento distinto pero estable durante toda esa hora.
            const semilla = hora + index;
            const aleatorio = Math.sin(semilla) * 10000;
            const valorFijo = aleatorio - Math.floor(aleatorio); // Número entre 0 y 1
